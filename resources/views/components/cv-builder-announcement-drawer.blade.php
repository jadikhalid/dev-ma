@php
    ...
    $cvPreviewImage = TalentCvMarketingPreview::imagePath(TalentCvDraft::TEMPLATE_CLASSIC);
    $cvPreviewDimensions = TalentCvMarketingPreview::imageDimensions(TalentCvDraft::TEMPLATE_CLASSIC);
@endphp

<div
    x-data="cvBuilderAnnouncement({ previewImage: @js($cvPreviewImage) })"
    x-show="open"
        ...
        >
            ...
            <div class="px-5 py-4 sm:px-6">
                <div class="flex flex-col sm:flex-row gap-4 sm:gap-5 items-start">
                    ...
                    <figure
                        class="w-full sm:w-[200px] shrink-0 overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm"
                        style="aspect-ratio: {{ $cvPreviewDimensions['width'] }} / {{ $cvPreviewDimensions['height'] }};"
                    >
                        <img
                            src="{{ $cvPreviewImage }}"
                            alt="{{ __('talenma.cv_builder.templates.classic') }}"
                            class="block h-full w-full object-cover object-top"
                            width="{{ $cvPreviewDimensions['width'] }}"
                            height="{{ $cvPreviewDimensions['height'] }}"
                            loading="eager"
                            fetchpriority="high"
                            decoding="sync"
                        >
