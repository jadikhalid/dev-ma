@props(['disabled' => false])

@if ($disabled)
    <span {{ $attributes->merge(['class' => 'block w-full px-4 py-2 text-start text-sm leading-5 text-gray-300 cursor-not-allowed select-none [&_svg]:text-gray-300', 'aria-disabled' => 'true']) }}>
        {{ $slot }}
    </span>
@else
    <a {{ $attributes->merge(['class' => 'block w-full px-4 py-2 text-start text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out']) }}>
        {{ $slot }}
    </a>
@endif
