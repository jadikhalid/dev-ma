...
<x-guest-layout :viewport-fit="! $pendingRegistrationEmail">
    ...
    >@csrf
        ...
        >
            ...
            <template x-if="isCompany">
                <div>
                    ...
                    <div class="flex items-center gap-2">
                        <span class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-colors" :class="step === 1 ? 'bg-emerald-600 text-white' : (step1Valid ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-400')">1</span>
                        <div class="flex-1 h-1 rounded-full bg-gray-200 overflow-hidden">
                            <div class="h-full bg-emerald-600 transition-all duration-300" :style="`width: ${step >= 2 ? '100%' : '0%'}`"></div>
                        </div>
                        <span class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-colors" :class="step === 2 ? 'bg-emerald-600 text-white' : (step > 2 ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-400')">2</span>
                        <div class="flex-1 h-1 rounded-full bg-gray-200 overflow-hidden">
                            <div class="h-full bg-emerald-600 transition-all duration-300" :style="`width: ${step === 3 ? '100%' : '0%'}`"></div>
                        </div>
                        <span class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-colors" :class="step === 3 ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-400'">3</span>
                    </div>
                    <p class="mt-1.5 text-sm sm:text-xs text-gray-500 line-clamp-2" x-text="step === 1 ? @js(__('talenma.auth.register_step_1_hint')) : (step === 2 ? @js(__('talenma.auth.register_company_step_2_hint')) : @js(__('talenma.auth.register_company_step_3_hint')))"></p>
                </div>
            </template>
        </div>
        ...
        <div x-show="hasRole" x-cloak class="shrink-0 mb-3 flex items-center justify-between gap-2">
            <span
                class="inline-flex items-center gap-1.5 text-base sm:text-sm font-semibold"
            ...
            <button
                type="button"
                @click="resetRole()"
                class="text-base sm:text-sm font-semibold underline underline-offset-2 transition-colors"
        ...
        <div class="flex-1 min-h-0 overflow-y-auto overscroll-contain -mx-1 px-1">
            ...
            >
                ...
                <div x-show="!hasRole" x-cloak>
                    <x-input-label :value="__('talenma.auth.register_as')" class="!text-base sm:!text-sm" />
                    <div class="mt-2 grid grid-cols-2 gap-3">
                        <label class="group flex flex-col items-center text-center gap-2 p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-colors has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50 hover:border-indigo-300">
                            ...
                            <span class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-indigo-100 text-indigo-600 group-has-[:checked]:bg-indigo-600 group-has-[:checked]:text-white transition-colors">
                            ...
                            <span class="font-semibold text-base sm:text-sm">{{ __('talenma.auth.role_talent') }}</span>
                            @if (__('talenma.auth.role_talent_desc'))
                                <p class="text-sm sm:text-xs text-gray-500 line-clamp-2">{{ __('talenma.auth.role_talent_desc') }}</p>
                            @endif
                        </label>
                        <label class="group flex flex-col items-center text-center gap-2 p-3 sm:p-4 border-2 rounded-xl cursor-pointer transition-colors has-[:checked]:border-emerald-600 has-[:checked]:bg-emerald-50 hover:border-emerald-300">
                            ...
                            <span class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-emerald-100 text-emerald-600 group-has-[:checked]:bg-emerald-600 group-has-[:checked]:text-white transition-colors">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0H5m14 0h2M5 21H3m6-14h.01M9 11h.01M9 15h.01M15 7h.01M15 11h.01M15 15h.01"/></svg>
                            </span>
                            <span class="font-semibold text-base sm:text-sm">{{ __('talenma.auth.role_company') }}</span>
                            @if (__('talenma.auth.role_company_desc'))
                                <p class="text-sm sm:text-xs text-gray-500 line-clamp-2">{{ __('talenma.auth.role_company_desc') }}</p>
                            @endif
                        </label>
                    </div>
                    <x-input-error :messages="$errors->get('role')" class="mt-1" />
                    <p class="mt-3 text-center text-sm sm:text-xs text-gray-500">
                ...
                >
                    <div x-show="isCompany" x-cloak>
                        <label for="name" class="block font-medium !text-base sm:!text-sm text-gray-700">
                            {{ __('talenma.auth.company_name') }}
                        </label>
                        <x-text-input id="name" name="name" x-model="name" @blur="onFieldBlur('name')" @input="onFieldInput('name')" x-bind:class="fieldInvalidClass('name')" class="mt-1.5 block w-full !text-base !py-3 sm:mt-1 sm:!text-sm sm:!py-2" minlength="2" maxlength="255" autocomplete="organization" x-bind:required="isCompany" x-bind:disabled="!isCompany" />
            ...
            >
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <x-input-label for="representative_first_name" :value="__('talenma.auth.representative_first_name')" class="!text-base sm:!text-sm" />
                        <x-text-input id="representative_first_name" name="first_name" x-model="firstName" @blur="onFieldBlur('first_name')" @input="onFieldInput('first_name')" x-bind:class="fieldInvalidClass('first_name')" class="mt-1.5 block w-full !text-base !py-3 sm:mt-1 sm:!text-sm sm:!py-2" minlength="2" maxlength="127" autocomplete="given-name" x-bind:required="isCompany" x-bind:disabled="!isCompany" />
                        <x-input-error :messages="$errors->get('first_name')" class="mt-1" />
                    </div>
                    <div>
                        <x-input-label for="representative_last_name" :value="__('talenma.auth.representative_last_name')" class="!text-base sm:!text-sm" />
                        <x-text-input id="representative_last_name" name="last_name" x-model="lastName" @blur="onFieldBlur('last_name')" @input="onFieldInput('last_name')" x-bind:class="fieldInvalidClass('last_name')" class="mt-1.5 block w-full !text-base !py-3 sm:mt-1 sm:!text-sm sm:!py-2" minlength="2" maxlength="127" autocomplete="family-name" x-bind:required="isCompany" x-bind:disabled="!isCompany" />
                        <x-input-error :messages="$errors->get('last_name')" class="mt-1" />
                    </div>
                </div>
                <div>
                    <x-input-label for="company_sector" :value="__('talenma.auth.sector')" class="!text-base sm:!text-sm" />
                    <select id="company_sector" name="sector" x-model="sector" @blur="onFieldBlur('sector')" @change="onFieldInput('sector')" x-bind:class="fieldInvalidClass('sector')" class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm !text-base !py-3 sm:!text-sm sm:!py-2">
                ...
                <div>
                    <x-input-label for="company_description" :value="__('talenma.company.description')" class="!text-base sm:!text-sm" />
                    <textarea
                    ...
                    <div class="mt-0.5 flex items-center justify-between gap-2 text-sm sm:text-xs text-gray-500">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <x-input-label for="company_website" :value="__('talenma.auth.company_website')" class="!text-base sm:!text-sm" />
                        <x-text-input id="company_website" name="company_website" type="url" x-model="companyWebsite" @blur="onFieldBlur('company_website')" @input="onFieldInput('company_website')" x-bind:class="fieldInvalidClass('company_website')" class="mt-1.5 block w-full !text-base !py-3 sm:mt-1 sm:!text-sm sm:!py-2" placeholder="https://..." />
                        <x-input-error :messages="$errors->get('company_website')" class="mt-1" />
                    </div>
                    <div>
                        <x-input-label for="company_country" :value="__('talenma.auth.company_country')" class="!text-base sm:!text-sm" />
                        <select
                            id="company_country"
                            ...
                            class="mt-1 block w-full border-gray-300 rounded-lg shadow-sm !text-base !py-3 sm:!text-sm sm:!py-2"
                        >
            ...
            >
                <div>
                    <x-input-label for="company_documents" :value="__('talenma.auth.company_registration_documents')" class="!text-base sm:!text-sm" />
                    <input
                        ...
                        class="mt-1 block w-full text-sm sm:text-sm text-gray-600 file:mr-3 file:py-2 file:px-3.5 file:rounded-lg file:border-0 file:text-sm sm:file:text-xs file:font-semibold file:bg-emerald-50 file:text-emerald-700 hover:file:bg-emerald-100"
                    ...
                    <p class="mt-0.5 text-sm sm:text-xs text-gray-500">{{ __('talenma.auth.company_registration_documents_hint') }}</p>
                ...
                <div class="rounded-lg border border-gray-200 bg-gray-50 px-3 py-3">
                    <label class="flex items-start gap-2.5 cursor-pointer">
                        <input
                            ...
                            class="mt-0.5 size-5 sm:size-4 rounded border-gray-300 text-emerald-600 shadow-sm focus:ring-emerald-500"
                        >
        ...
        <div class="shrink-0 pt-3 mt-3 border-t border-gray-100 flex items-center gap-2 sm:gap-3">
            ...
            >
                <svg class="w-5 h-5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
            ...
            <div class="flex-1 flex justify-end min-w-0">
                <button
                    ...
                    :class="canGoNext
                        ? (isCompany
                            ? 'inline-flex items-center gap-1.5 px-4 py-3 sm:py-2 text-base sm:text-sm font-semibold text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 transition shadow-sm'
                            : 'inline-flex items-center gap-1.5 px-4 py-3 sm:py-2 text-base sm:text-sm font-semibold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 transition shadow-sm')
                        : 'inline-flex items-center gap-1.5 px-3.5 py-3 sm:px-3 sm:py-2 text-base sm:text-sm font-semibold text-gray-300 border border-gray-200 rounded-lg cursor-not-allowed bg-gray-50'"
                >
                    {{ __('talenma.auth.register_continue') }}
                    <svg class="w-5 h-5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </button>
