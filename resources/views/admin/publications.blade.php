<x-app-layout>
    ...
    >
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6 xl:gap-8 items-start">
            ...
            <section id="reseaux" class="scroll-mt-24 space-y-4 min-w-0">
                ...
                @if (session('post_updated'))
                    <div class="p-3 bg-green-50 border border-green-200 text-green-800 rounded-xl text-sm">
                        {{ __('talenma.admin.social_posts.updated') }}
                    </div>
                @endif
        ...
        {{-- Bottom sheet: modifier un post --}}
        <div
            x-show="socialEditOpen"
            x-cloak
            class="fixed inset-0 z-[60]"
            role="dialog"
            aria-modal="true"
            aria-labelledby="publications-social-edit-title"
        >
            <div
                x-show="socialEditOpen"
                x-transition:enter="transition-opacity ease-out duration-300"
            ...
            >
                ...
                <div class="flex shrink-0 items-start justify-between gap-4 border-b border-gray-100 px-5 py-4">
                    <div class="min-w-0">
                        <p class="text-xs font-semibold uppercase tracking-wide text-indigo-600">{{ __('talenma.admin.social_posts.title') }}</p>
                        <h3 id="publications-social-edit-title" class="mt-1 text-lg font-bold text-gray-900">{{ __('talenma.admin.social_posts.edit_title') }}</h3>
                    </div>
                    <button
                        type="button"
                        class="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600"
                ...
                <div class="min-h-0 flex-1 overflow-y-auto px-5 py-5">
                    <div id="publications-social-edit-card" class="relative">
                        <form
                            id="publications-social-edit"
                            method="POST"
                            :action="socialEditAction"
                            enctype="multipart/form-data"
                            class="space-y-4"
                            data-ajax
                            data-refresh="publications-social"
                            data-loading-target="publications-social-edit-card"
                            data-error-message="{{ __('talenma.common.save_error') }}"
                            novalidate
                        >
                            @csrf
                            ...
                            <div>
                                <x-input-label for="post_edit_title" :value="__('talenma.admin.social_posts.field_title')" />
                                <x-text-input
                                    id="post_edit_title"
                                    name="post_title"
                                    class="mt-1 block w-full"
                                    x-model="socialEditTitle"
                                    required
                                    data-required
                                    data-required-message="{{ __('talenma.admin.social_posts.title_required') }}"
                                />
                            ...
                            <div>
                                <x-input-label for="post_edit_subtitle" :value="__('talenma.admin.social_posts.field_subtitle')" />
                                <x-text-input
                                    id="post_edit_subtitle"
                                    name="post_subtitle"
                                    class="mt-1 block w-full"
                                    x-model="socialEditSubtitle"
                                    required
                                    data-required
                                    data-required-message="{{ __('talenma.admin.social_posts.subtitle_required') }}"
                                />
                            ...
                            <div>
                                <x-input-label for="post_edit_url" :value="__('talenma.admin.social_posts.field_url')" />
                                <x-text-input
                                    id="post_edit_url"
                                    name="post_url"
                                    type="url"
                                    class="mt-1 block w-full"
                                    x-model="socialEditUrl"
                                    required
                                    data-required
                                    data-required-message="{{ __('talenma.admin.social_posts.url_required') }}"
                                    data-url
                            ...
                            <div>
                                <x-input-label for="post_edit_network" :value="__('talenma.admin.social_posts.field_network')" />
                                <select
                                    id="post_edit_network"
                                    name="post_network"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                                    x-model="socialEditNetwork"
                                    required
                                    data-required
                                    data-required-message="{{ __('talenma.admin.social_posts.network_required') }}"
                                >
                            ...
                            <div>
                                <x-input-label for="post_edit_thumbnail" :value="__('talenma.admin.social_posts.field_thumbnail')" />
                                <input
                                    id="post_edit_thumbnail"
                                    name="post_thumbnail"
                                    type="file"
                                    accept="image/*"
                                    class="mt-1 block w-full text-sm text-gray-600 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-indigo-50 file:text-indigo-700 file:font-semibold hover:file:bg-indigo-100"
                                >
                                <p class="mt-1 text-xs text-gray-500">{{ __('talenma.admin.social_posts.thumbnail_keep') }}</p>
                            </div>
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 sm:justify-end pt-1">
                                <button
                                    ...
                                    @click="closeSocialEdit()"
                                >{{ __('talenma.common.cancel') }}</button>
                                <x-primary-button class="justify-center">{{ __('talenma.admin.social_posts.update') }}</x-primary-button>
                            </div>
                        </form>
                    </div>
                </div>
