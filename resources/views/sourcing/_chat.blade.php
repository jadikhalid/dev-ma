@php
    $viewer = Auth::user();
    $canChat = $recruitment->allowsChat();
    $sidebar = $sidebar ?? true;
    $storeRoute = $viewer->isStaff()
        ? route('admin.recruitment.messages.store', $recruitment)
        : route('sourcing.messages.store', $recruitment);
    $peerName = $viewer->isStaff()
...
>
    <header class="shrink-0 border-b border-indigo-100 bg-gradient-to-r from-indigo-50 via-white to-slate-50 px-4 sm:px-5 py-3.5">
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0 flex-1">
                <h3 class="text-sm font-bold tracking-tight text-slate-900">{{ __('talenma.recruitment.chat_title') }}</h3>
                <p class="text-xs text-slate-500 truncate">{{ __('talenma.recruitment.chat_with', ['name' => $peerName]) }}</p>
            </div>
            @if ($canChat)
                <span class="shrink-0 inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-emerald-700 ring-1 ring-emerald-100">
                    ...
                    {{ __('talenma.recruitment.chat_open_badge') }}
                </span>
            @else
                <span class="shrink-0 inline-flex items-center rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-slate-600 ring-1 ring-slate-200">
                    {{ __('talenma.recruitment.chat_closed_badge') }}
                </span>
            @endif
        </div>
    </header>
    ...
    >
        @forelse ($recruitment->messages as $msg)
            @include('sourcing._chat-message', [
                'msg' => $msg,
        ...
        @empty
            <div data-chat-empty class="flex h-full min-h-[10rem] flex-col items-center justify-center px-4 text-center">
                ...
                <p class="text-sm font-medium text-slate-700">{{ __('talenma.recruitment.chat_empty') }}</p>
                <p class="mt-1 text-xs text-slate-500">{{ __('talenma.recruitment.chat_empty_hint') }}</p>
            </div>
        @endforelse
    </div>
    ...
    <div>
            ...
            >
                ...
                <div class="flex items-end gap-2 rounded-2xl bg-slate-50 p-2 ring-1 ring-slate-200 focus-within:ring-2 focus-within:ring-indigo-400 focus-within:bg-white transition">
                    <textarea
                        ...
                        placeholder="{{ __('talenma.recruitment.chat_placeholder') }}"
                    ></textarea>
                    <button
                        type="submit"
                        ...
                        title="{{ __('talenma.recruitment.chat_send') }}"
                    >
                        <span class="sr-only">{{ __('talenma.recruitment.chat_send') }}</span>
                ...
                <div class="mt-2 flex flex-wrap items-center justify-between gap-2">
                    <p class="text-[11px] text-slate-400">{{ __('talenma.recruitment.chat_composer_hint') }}</p>
                    <p
        ...
        @else
            <div class="shrink-0 border-t border-slate-200 bg-slate-50 px-4 sm:px-5 py-3.5">
                <p class="text-xs text-slate-500">{{ __('talenma.recruitment.chat_closed') }}</p>
            </div>
        @endif
    </div>
</section>
