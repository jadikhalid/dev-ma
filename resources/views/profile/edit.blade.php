@php
    ...
    $showTalentPanel = $showTalentPanel ?? false;
    $panel = $panel ?? 'account';
    $showSettingsShell = $showCompanyPanel || $showTalentPanel;
@endphp
        ...
        <div class="flex items-start gap-3">
            ...
            <div>
                <h2 class="text-xl font-bold text-gray-900">
                    {{ $showSettingsShell ? __('talenma.account.settings_title') : __('talenma.account.title') }}
                </h2>
                <p class="text-sm text-gray-500 mt-0.5">
                    @if ($showCompanyPanel)
                        {{ __('talenma.account.settings_subtitle') }}
                    @elseif ($showTalentPanel)
                        {{ __('talenma.account.settings_subtitle_talent') }}
            ...
            @if ($showSettingsShell)
                <div class="flex flex-col lg:flex-row gap-8">
                        ...
                        <nav class="flex lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-1 lg:pb-0" aria-label="{{ __('talenma.account.settings_nav') }}">
                            ...
                            @if ($showTalentPanel)
                                <a
                                    ...
                                    @class([
                                        'inline-flex items-center gap-2 whitespace-nowrap rounded-xl px-3.5 py-2.5 text-sm font-semibold transition',
                                        'bg-indigo-50 text-indigo-800' => $panel === 'talent',
                                        'text-gray-600 hover:bg-gray-50 hover:text-gray-900' => $panel !== 'talent',
                                    ])
                                >
                                    <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" stroke-width="1.75" viewBox="0 0 24 24" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z"/></svg>
                                    {{ __('talenma.nav.my_profile') }}
                                </a>
                            @endif
                        </nav>

                        @if ($panel === 'company' && isset($profile) && $showCompanyPanel)
                    ...
                    <div class="min-w-0 flex-1 space-y-6">
                        ...
                        @elseif ($panel === 'talent' && $showTalentPanel)
